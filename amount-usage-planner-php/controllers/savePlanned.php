...
function formatArray($amount_type, $amountCount, $amount, $plan_percentage, $month, $year, $SAVE_PLAN_CHECK) {
...
$checkEachMonthAmountSaveQuery = "SELECT count(*) `count` FROM amount WHERE `month` = '". $month."' AND `year` = " . $year . " ";
...
if($monthAmountCount == 1) {
    ...
    if(isset($UpdateData_month)) {
        /** This is for the Situation
         *      When Change in ITEMS
         * ********************************************* This Will Change Too - According to Month User is Updating
         */
        if($save_planCount != 0 && $save_planCount != count($plan_percentage)) { /** Means there should be Data In it */
            $db->rawSQLQuery('DELETE from `amount` where `month` = '.$UpdateData_month);
        }
        $SAVE_PLAN_CHECK = true; /** THIS is when Any Update in SAVE Plan appears */
    }
    ...
    /** Just For 1 User */
    // if($amountCount == 0) {
        formatArray($amount_type, $amountCount, $amount, $plan_percentage, $month, $year, $SAVE_PLAN_CHECK);
    // }
}
